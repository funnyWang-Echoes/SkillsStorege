...
| Skill | 作用 | 来源 | 可用程度 |
|---|---|---|---|
| `aihot` | 查询 AI HOT 中文 AI 资讯、日报、精选条目和关键词动态。 | [KKKKhazix/khazix-skills](https://github.com/KKKKhazix/khazix-skills) | 外部可用；当前上游版本 **v1.5.4**（2026-08-26 同步），frontmatter 含 `metadata.version` 与 License (MIT)；SKILL.md 新增「安全边界」「用途许可边界」两节；仓库附 `install.sh`、`agents/openai.yaml`、`manifest.sha256`、`references/{api,errors,sync}.md`。依赖 `aihot.virxact.com` 公共 v1 API。 |
...
| `neat-freak` | 会话结束后对项目文档、AGENTS/CLAUDE 规则和记忆做同步清理。 | [KKKKhazix/khazix-skills](https://github.com/KKKKhazix/khazix-skills) | 外部可用，适合阶段收尾。当前上游版本 **v3.0**（2026-08-26 同步，commit `2b4a645 neat-freak v3.0`），description 改为 "Knowledge and governance closeout"，新增 middle-tier triggers、light path for small projects、generic platform fallback；附带 `evals/` 完整评测套件（11 个 fixture project）与 `scripts/audit-inventory.sh`。 |
...
| `web-design-engineer` | 生成高质量网页、仪表盘、原型、交互演示和数据可视化。 | [ConardLi/garden-skills](https://github.com/ConardLi/garden-skills) | 外部可用；适合视觉前端产物。当前上游版本 **v1.3.0**（2026-08-26 同步，commit `0886299 docs(readme): sync download links for web-design-engineer v1.3.0`），manifest 1.2.2 → 1.3.0；新增 five-dial Design Read、preservation-aware redesigns、contextual failure patterns；`references/` 增 5 个文档（block-library / browser-acceptance / design-calibration / failure-patterns / redesign-protocol）。 |
...
| `ppt-master` | 多角色协作式 PPT 生成系统，把 PDF/DOCX/URL/Markdown 转为 SVG 页面并导出 PPTX。 | [hugohe3/ppt-master](https://github.com/hugohe3/ppt-master)；MIT；作者 `hugohe3`；**真实 Skill 路径为 `skills/ppt-master/SKILL.md`（不在仓库根）**；首次入库 commit `4e57f2f7`（2026-03-20，refactor: migrate to skill-based architecture）；当前上游 HEAD `ebd74d1f`（2026-08-25），版本 **v5.0.0**。 | 外部可用；能力完整但体量较大（~95M / 12,925 文件），使用前应按其依赖和流程做实测。**v5.0.0 架构变化（2026-08-26 同步）**：从 v2 串行 pipeline（542 行 SKILL.md）改为路由式架构（79 行 SKILL.md + `workflows/routing.md` + `workflows/profiles/*` + `workflows/stages/*` + `workflows/governance/*`）。SKILL.md 第 2 步强制调用 `scripts/attribution_guard.py`（已随同步入库）；新增 `pptx-shape-authoring`、`pptx-effects`、`pptx-gradients`、`pptx-transitions` 等原生 PPTX 能力；`templates/` 增 `styles/`、`decks/`、`scaffolds/`、`schemas/`、`sounds/` 5 类；`brands/` 从 12 增至 18。**架构变更**：v2 时代的 8 个工作流文件（create-brand、customize-animations、generate-audio、live-preview、resume-execute、topic-research、verify-charts、visual-review）在 v5 中被路由表替代，调用方式显著变化。 |
...
|---|---|---|---|
| 2026-06-14 | [hugohe3/ppt-master](https://github.com/hugohe3/ppt-master) | `4e57f2f7`（2026-03-20，`refactor: migrate to skill-based architecture` — **首次入库 commit，已修正历史错误 `a0d6243`**） | `ppt-master`（v2 时代串行 pipeline；542 行 SKILL.md） |
...
| 2026-08-26 | [KKKKhazix/khazix-skills](https://github.com/KKKKhazix/khazix-skills) | `7a5c493`（HEAD） | `neat-freak` 升至 v3.0（+evals/ 11 fixture、`references/{governance,verification}.md`、`scripts/audit-inventory.sh`）、`aihot` 升至 v1.5.4（+`README.md`/`LICENSE`/`install.sh`/`manifest.sha256`/`references/{api,errors,sync}.md`、`agents/openai.yaml`，frontmatter 加 `metadata.version`） |
| 2026-08-26 | [ConardLi/garden-skills](https://github.com/ConardLi/garden-skills) | `aaf9a82`（HEAD） | `web-design-engineer` 升至 v1.3.0（manifest 1.2.2 → 1.3.0；+`agents/openai.yaml`；`references/` 增 5 个文档：block-library / browser-acceptance / design-calibration / failure-patterns / redesign-protocol） |
| 2026-08-26 | [Vincentwei1021/video-shotcraft](https://github.com/Vincentwei1021/video-shotcraft)；Apache-2.0 | `d9ffa6d3`（HEAD） | `video-shotcraft` 同步到上游 HEAD；新增 `demos/` 镜头参考实现、`gallery/` 海报与源、`jianying-export/` 剪映工程导出（5 文件）、`assets/lib/ClipCard.tsx`、`assets/scripts/smoke-render-demos.py`、`package.json`/`package-lock.json`；副本 ~53M / 889 文件（已排除上游 `.git/` 184M） |
| 2026-08-26 | [hugohe3/ppt-master](https://github.com/hugohe3/ppt-master)；MIT；作者 `hugohe3` | `ebd74d1f`（HEAD，v5.0.0） | `ppt-master` 全目录同步到上游 `skills/ppt-master/` 子目录；**架构级重写**：v2 串行 pipeline（542 行 SKILL.md）→ v5 路由式（79 行 + `workflows/routing.md` + `profiles/` + `stages/` + `governance/`）；新增 `scripts/attribution_guard.py`（SKILL.md 第 2 步硬性 gate）、15 个 references、5 类新模板（styles/decks/scaffolds/schemas/sounds）、6 个 brand；副本 ~95M / 12,925 文件（仅拷贝 Skill 子目录，未拷上游仓库根） |
