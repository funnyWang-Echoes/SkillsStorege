---
name: autoresearch
description: 在当前项目里做自主实验研究循环——agent 反复提出改动、跑实验、读指标、keep/discard、advance/reset，直到预算耗尽或达标。泛化自 karpathy/autoresearch，把训练场景抽成可配置的「项目研究配置」，适用于任何有明确单一指标、可自动运行、可 git 版本化的优化任务（模型评测、仿真调参、算法/超参搜索、性能优化的多轮迭代）。触发词："autoresearch"、"自主研究"、"通宵跑实验"、"自动迭代优化"、"让 agent 自己一直试"、"过夜实验循环"。不用于单次性能优化、没有自动化指标的探索性任务、需要人逐步决策的研究。
version: "0.2.0-local"
---
...
**不用：**
- 单次或少数几次工程性能优化 → 不必起循环，按常规工程优化流程处理（改动少、人盯得住）。
...
2. **运行命令（run command）**：一条自动跑完并把结果重定向到日志的命令，如 `uv run train.py > run.log 2>&1`。用重定向而非 tee，避免输出灌爆 context。命令按目标环境的 shell 写：Linux/macOS 用 bash；Windows PowerShell 重定向语义不同（默认 UTF-16、不支持 `&&`），建议统一在 bash 或远程 Linux 环境执行。
...
7. **结果目录**：建 `results/` 存产物，把 `assets/log-template.md` 复制为 `results/log.md`，填好顶部研究配置、清空示例行。baseline 第一次跑后记录。同时约定 `results/` 的 git 口径：文本台账（`log.md`、调研笔记）commit 进分支；`run.log` 及大二进制产物按体量 `.gitignore` 或另存，避免仓库膨胀。

复述配置，用户确认后开始。

## LOOP：实验循环

确认配置后进入循环。**第一轮永远是 baseline：原样跑，不改任何东西，记录基准。这一轮同时验证抓取命令**：baseline 跑完用抓取命令读指标，读不到 = 抓取命令写错，先修好再进循环，否则后续每轮都会被误判成 crash。
...
现有材料耗尽时主动联网调研找新方向：相关论文、SOTA 做法、开源实现的 trick。把调研结论落盘到 `results/research/<日期>-<主题>.md`（如 `results/research/2026-03-05-muon-variants.md`），记下来源、可试的具体改动、与当前代码的差异。调研得出的想法进入 LOOP 后，在该实验的 `log.md` 记录里注明来源（哪篇调研/哪个来源），形成「调研 → 想法 → 实验」的可追溯链。

### 中断恢复（断线 / 重启后续跑）

循环靠 git 分支 + `results/log.md` 做记忆，中断后可直接恢复，不重头跑：

1. 确认当前分支仍是 `autoresearch/<tag>`；`git status` 若有未 commit 的脏改动，按 discard 处理（reset 掉）——中断的那轮没有完整结果。
2. 读 `results/log.md` 最新总览行，确认最后一轮状态与当前 best。
3. 从下一轮编号继续 LOOP，不重跑 baseline。
