...
本仓库用于用 Git/GitHub 管理个人自写与从他人 GitHub 下载的 Skill 和 MCP server。Agent 在这里工作时，目标不是收集整仓素材，而是维护一份可直接安装/运行、可追溯来源、可持续迭代的 Skill 与 MCP 库。
...
- `other-MCPs/`：只放他人来源的 MCP server 本体。
...
每个 MCP 目录必须直接包含包清单与入口（如 `pyproject.toml`/`package.json` 加源码目录）。如果 MCP 本体藏在外层仓库的子目录里，只复制那个真正的 MCP 子目录。
...
每次从 GitHub 下载他人的 MCP，流程与 Skill 相同，判定与记录标准换成 MCP 的等价物：

1. 记录 GitHub 仓库 URL、作者、License、README 摘要、下载日期、包发布名（如 PyPI/npm 名）与拉取版本/commit。
2. 找到真实 MCP 本体：以是否直接包含包清单（`pyproject.toml`/`package.json` 等）加可运行源码为准。
3. 只把真实 MCP 本体复制到 `other-MCPs/<mcp-name>/`，目录名优先用能唯一识别来源的名字（如 `<作者>-<项目名>` 或发布包名）。
4. 删除外层仓库壳、demo、website、release 脚本、CI 配置等非本体内容；源码自带的测试保留。
5. 入库前做基本验证（如跑测试、stdio 握手冒烟），并把来源、作用、可用程度、平台限制（如仅 POSIX）、安全注意事项写入根目录 `Readme.md`。
6. MCP 会执行命令、访问网络或凭据时，必须审查其安全声明是否与代码一致，发现不符要在 `Readme.md` 明确记录。
...
- 真实 Skill/MCP 本体已复制到 `other-Skills/` 或 `other-MCPs/`
- 根 README 已记录来源信息
- 删除路径位于本仓库根目录下

不要删除 `my-Skills/`、`other-Skills/` 或 `other-MCPs/` 中的本体，除非用户明确要求。
