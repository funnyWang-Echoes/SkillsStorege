...
def get_token(env_path: Path, token_env: str) -> str:
    """从进程环境或 .env 文件读取 MinerU API token。"""
    token = os.environ.get(token_env)
    if token:
        return token
    token = load_env_file(env_path).get(token_env, "")
    if not token:
        raise RuntimeError(f"MinerU token not found: set {token_env} or provide --env {env_path}")
    return token
