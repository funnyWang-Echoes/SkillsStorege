...
def get_user_data_dir(account: Optional[str] = None) -> str:
    ...
    """
    try:
        from account_manager import get_profile_dir  # 第五轮 P2 fix：account_manager 已删，silently 走 fallback
...
def verify_chrome_cdp(port: int = CDP_PORT) -> bool:
    """第五轮 P2 fix：端口被占时探测是不是真正的 Chrome CDP（避免误连别处）。

    通过 /json/version 端点验证响应是 Chrome 返回的（其他服务也可能占 9222）。
    """
    try:
        import requests
...
def launch_chrome(
    ...
    if is_port_open(port):
        if verify_chrome_cdp(port):
            print(f"[chrome_launcher] Chrome already running on port {port}.")
            return None
        # 第五轮 P2 fix：端口被占但不是 Chrome，可能是别的服务
        print(f"[chrome_launcher] [ERR] 端口 {port} 已被占用，但不是 Chrome CDP。")
        print(f"      请用 --port=9223 或 --kill 杀掉占用进程后重试。")
        sys.exit(1)
