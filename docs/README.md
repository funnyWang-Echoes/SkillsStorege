...
| [source-verify-archify-2026-09-19.md](source-verify-archify-2026-09-19.md) | source-verify | 2026-09-19 | `tt-a1i/archify` 来源核验通过（MIT、main HEAD `72c750bb`、本体在 `archify/` 子目录）；按用户决策**只登记不入库**，登记按需拉取命令；本机已装副本与 HEAD 的 git blob 哈希比对：63 一致 / 16 落后 / 139 缺失（`test/` 与 `package-lock.json` 属发行裁剪）。 |
| [source-verify-grill-me-2026-09-19.md](source-verify-grill-me-2026-09-19.md) | source-verify | 2026-09-19 | `grill-me` / `grilling` **来源待确认**：本机唯一副本在 `C:\Users\c\.codex\skills\`（`~/.zcode/skills/grill-me` 是指向它的符号链接），frontmatter 无来源字段，公开检索未命中同文本（已排除 `RobMitt/grill-me-skill`、`joshuawheelock/grill-me`）；连带发现 ZCode 侧未链接 `grilling`，`/grilling` 为悬空引用。 |
| [sync-harness-installs-2026-09-19.md](sync-harness-installs-2026-09-19.md) | sync | 2026-09-19 | 核验本机安装拓扑（ZCode 侧是指向仓库的符号链接，Codex 侧是实体副本）；把 Codex 侧 4 个过期 Skill（`aihot`/`neat-freak`/`web-design-engineer`/`ppt-master`）重同步为与仓库逐字节一致；覆盖前逐个判定无本地独有内容（142 个独有文件全部是 v2 时代上游文件，可由 `089f648` 与 `7d5eeda` 佐证）；备份 65 MB 在 `skills-backup-2026-09-19`。同时纠正早前"安装副本与仓库一致"的无效比对。 |